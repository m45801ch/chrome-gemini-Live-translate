...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ onConfigChange }) => {
  ...
  const updateHistoricalColor = (type: "main" | "translation", color: string) =>
    updateConfig((prev) => ({
      ...prev,
      subtitleStyle: {
        ...prev.subtitleStyle,
        historical: {
          ...prev.subtitleStyle.historical,
          [type]: { color },
        },
      },
    }));
  ...
  return (
    <div style={panelStyle}>
      ...
      {/* ── 歷史字幕（原文） ── */}
      <Section title="歷史字幕樣式（原文灰化行）">
        <Field label="文字顏色">
          <ColorPicker value={s.historical.main.color} onChange={(c) => updateHistoricalColor("main", c)} />
        </Field>
      </Section>

      {/* ── 歷史字幕（譯文） ── */}
      <Section title="歷史字幕樣式（譯文灰化行）">
        <Field label="文字顏色">
          <ColorPicker value={s.historical.translation.color} onChange={(c) => updateHistoricalColor("translation", c)} />
        </Field>
      </Section>
    </div>
  );
