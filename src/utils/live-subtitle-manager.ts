...
export class LiveSubtitleManager {
  ...
  addChunk({ original, translation, isFinal }: { original: string, translation: string, isFinal: boolean }) {
    ...
    if (last.isFinal) {
      ...
      // English sentence-boundary segmentation (period / question mark / exclamation)
      const origBoundary = last.original.match(/[.?!](?:\s|$)/)
      if (origBoundary && origBoundary.index !== undefined) {
        const origSplitIdx = origBoundary.index + 1
        const leftOriginal = last.original.substring(0, origSplitIdx)
        const rightOriginal = last.original.substring(origSplitIdx).trimStart()

        let leftTranslation = last.translation
        let rightTranslation = ""
        if (last.translation.length > 0 && rightOriginal) {
          const splitRatio = origSplitIdx / last.original.length
          const transEstIdx = Math.floor(splitRatio * last.translation.length)
          leftTranslation = last.translation.substring(0, transEstIdx)
          rightTranslation = last.translation.substring(transEstIdx)
        }
